...
// Cache regex's
var rImages = /([\s\S]*?)(url\(([^)]+)\))(?!\s*[;,]?\s*\/\*\s*CssUrlRewrite:skip\s*\*\/)|([\s\S]+)/gim; // TODO: Strip of CssUrlRewrite:skip
var rExternal = /^(http|\/\/)/;
var rData = /^data:/;
var rQuotes = /['"]/g;
var rParams = /([?#].*)$/g;
    ...
    async.whilst(
      ...
      function (complete) {
        ...
        if (group[4] == null) {
          ...
          params = group[3].replace(rQuotes, '').match(rParams);

          img = group[3].trim().replace(rQuotes, '').replace(rParams, ''); // remove query string/hash parmams in the filename, like foo.png?bar or foo.png#bar
          ...
          if (is_local_file) {
            ...
            loc = img.charAt(0) === '/' ? (opts.baseDir || '') + loc : path.join(path.dirname(srcFile), img).replace(/\\/g, '/');
